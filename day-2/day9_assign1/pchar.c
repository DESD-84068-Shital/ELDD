...
static __init int pchar_init(void) {
    ...
    for(i=0; i<devcnt; i++)
        //sema_init(&devices[i].lock);
        sema_init(&lock, 1);
    printk(KERN_INFO "%s: semaphore_init() initialized mutex lock for all devices.\n", THIS_MODULE->name);
...
static __exit void pchar_exit(void) {
    ...
    printk(KERN_INFO "%s: semaphore_destroy() destroyed mutex locks for all devices.\n", THIS_MODULE->name);
